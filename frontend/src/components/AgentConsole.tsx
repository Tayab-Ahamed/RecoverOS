...
export function AgentConsole({ selected, onInspect }: { selected: Case | null; onInspect: () => void }) {
  ...
  return <aside className={`agent-console ${isVerified ? "verified-console" : isStopped ? "stopped-console" : ""}`}>
    ...
    <div className="decision-theatre">
      <div className="theatre-title"><span>DECISION THEATRE</span><small>evidence → restraint → proof</small></div>
      <div className="theatre-step done"><i>01</i><div className="step-body"><strong>Diagnose</strong><span>{diagnosis?.cause?.replaceAll("_", " ") ?? "pending"}</span></div><b>sealed</b></div>
      <div className="theatre-step active"><i>02</i><div className="step-body"><strong>Propose</strong><span>{plan?.intervention?.replaceAll("_", " ") ?? "no plan"}</span></div><b>{plan?.is_llm_output ? "model" : "planner"}</b></div>
      <div className={`theatre-step ${isVerified ? "done" : "active"}`}><i>03</i><div className="step-body"><strong>{isStopped ? "Restraint" : "Authorize"}</strong><span>{isStopped ? (selected.state === "INELIGIBLE" ? "no contact" : "human handoff") : "policy gate"}</span></div><b>{isStopped ? "blocked" : "allowed"}</b></div>
      <div className={`theatre-step ${isVerified ? "done" : ""}`}><i>04</i><div className="step-body"><strong>Verify</strong><span>{selected.evidence?.captured ? "signed capture" : "awaiting proof"}</span></div><b>{isVerified ? "proven" : "open"}</b></div>
    </div>
